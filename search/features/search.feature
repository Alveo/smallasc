Feature: Search for items
    ...
    Scenario: Search results includes the appropriate heading
        After I login into the portal
        Given I access the url "/search/participants/components/?gender=male&ses=Professional&highest_qual=Bachelor+Degree&prof_cat=manager+and+admin&components_field=http%3A%2F%2Fid.austalk.edu.au%2Fcomponent%2F4_540_1_3&participants_field=4_540&speaker_no_field=2"
    ...
    Scenario: Item search must provide an option to limit result by speakers
        After I login into the portal
        Given I access the url "/search/participants/components/?gender=male&ses=Professional&highest_qual=Bachelor+Degree&prof_cat=assoc+professional"
        Then I see that label 27 is "Limit result set to"